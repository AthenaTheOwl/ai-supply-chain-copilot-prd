"""Validate decision records (DEC-*.md) in ai-supply-chain-copilot-prd.

Walks every `decisions/DEC-*.md`, parses the YAML front-matter, and
validates the parsed object against the cross-repo
`decision.schema.json` sourced from athena-site. The schema is fetched
from
...
def load_remote_schema() -> dict[str, Any] | None:
    try:
        req = urllib.request.Request(  # noqa: S310
            REMOTE_URL,
            headers={"User-Agent": "ai-supply-chain-copilot-prd/validate_decisions"},
        )
